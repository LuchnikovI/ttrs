  ...
  Ptr: PointerExt,
{
  ...
  ) -> MatrixResult<(Matrix<Ptr, Ref>, Matrix<Ptr, Ref>)>
  {
    ...
    let rhs_ptr = unsafe { self.ptr.add(self.ld * ncols) };
    let lhs_ncols = ncols;
    let rhs_ncols = self.ncols - ncols;
    Ok((
      Matrix { ptr: lhs_ptr, ncols: lhs_ncols,
               nrows: self.nrows, ld: self.ld,
               is_init: self.is_init, pref: PhantomData },
      Matrix { ptr: rhs_ptr, ncols: rhs_ncols,
               nrows: self.nrows, ld: self.ld,
               is_init: self.is_init, pref: PhantomData },
    ))
  }
  ...
  ) -> MatrixResult<(Matrix<Ptr, Ref>, Matrix<Ptr, Ref>)>
  {
    ...
    let lhs_ptr = self.ptr;
    let rhs_ptr = unsafe { self.ptr.add(nrows) };
    let lhs_nrows = nrows;
    let rhs_nrows = self.nrows - nrows;
    Ok((
      Matrix { ptr: lhs_ptr, ncols: self.ncols,
               nrows: lhs_nrows, ld: self.ld,
               is_init: self.is_init, pref: PhantomData },
      Matrix { ptr: rhs_ptr, ncols: self.ncols,
               nrows: rhs_nrows, ld: self.ld,
               is_init: self.is_init, pref: PhantomData },
    ))
  }
  ...
  ) -> MatrixResult<Matrix<Ptr, Ref>>
  {
    ...
    if self.ld != self.nrows {return Err(MatrixError::ImpossibleToReshape); }
    self.ld = nrows;
    self.nrows = nrows;
    self.ncols = ncols;
    Ok(self)
    ...
    T: Send + Sync + 'a,
  {
    ...
    (0..(nrows * ncols)).into_par_iter().map(move |i|{
      unsafe { ptr.add((i / nrows) * ld + (i % nrows)).deref() }
    })
  }
    ...
    T: Send + Sync + 'a,
  {
    ...
    let ld = self.ld;
    (0..(nrows * ncols)).into_par_iter().map(move |i|{
      unsafe { ptr.add((i / nrows) * ld + (i % nrows)).deref_mut() }
    })
  }
}

impl<T> From<&[T]> for Matrix<*const T, &[T]>
{

  fn from(buff: &[T]) -> Self {
    let ptr = buff.as_ptr();
    let len = buff.as_ref().len();
    Self { ptr, ncols: 1, nrows: len, ld: len, is_init: true, pref: PhantomData }
  }
}

impl<T> From<&mut [T]> for Matrix<*mut T, &mut[T]>
{
  ...
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    for i in 0..self.nrows {
      ...
      for j in 0..self.ncols{
        msg.push_str(&format!("{:?}, ", unsafe { self.ptr.add(j * ld + i).deref() }));
      }
      msg.push_str("]\n");
    }
    write!(f, "{}", msg)
